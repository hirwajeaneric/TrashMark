...
const StatsForWest = () => {
  ...
  useEffect(() => {
    getAllProductsRequest()
      .then((response) => {
        ...
        const monthlyRenewableTrash = response.products.filter((product) =>  product.category === "Renewable" && product.province === 'West');
        const monthlyNonRenewableTrash = response.products.filter((product) =>  product.category === "Non-renewable" && product.province === 'West');
  ...
  return (
    <div className="w-full rounded-md bproduct bproduct-gray-300 p-4 pt-0">
      <h2 className="text-sm font-bold mb-2">All recorded trash in Western province for {new Date().getFullYear()} in each month.</h2>
      <LineChart
